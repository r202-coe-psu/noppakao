...
@login_required
def index():
    submit_flags = models.Challenge.objects()
...
def create_or_edit(submit_flag_id):
    upload_data = models.Challenge.objects()
    form = forms.flags.UploadDataForm()
    categories = models.Category.objects(status="active")

    if submit_flag_id:
        upload_data = models.Challenge.objects.get(id=submit_flag_id)
...
def delete(submit_flag_id):
    submit_flag = models.Challenge.objects.get(id=submit_flag_id)
    submit_flag.delete()
    return redirect(url_for("upload_data.index"))
