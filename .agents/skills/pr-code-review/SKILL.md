...
# PR Code Review Guidelines (Noppakao CTF)
...
Resolve `{owner}` and `{repo}` from the current git remote (`gh repo view --json nameWithOwner`) or `git remote get-url origin`. Example: `psu__noppakao__pr-42.md`.
...
---
project_owner: psu
project_repo: noppakao
project_full_name: psu/noppakao
pr_number: 42
pr_title: "feat(challenge): add crypto challenge module"
pr_url: https://github.com/psu/noppakao/pull/42
...
workspace: /home/user/Projects/noppakao
---

## PR Review — feat(challenge): add crypto challenge module
...
## 1. Architecture & Data Flow (Flask & MongoDB)

- **MVC / Blueprints**: Are routes properly registered using Flask Blueprints? Are handlers kept relatively thin by moving business logic to dedicated modules?
- **MongoEngine**: Are database models defined correctly using `mongoengine`? Are relationships (`ReferenceField`) and embedded documents (`EmbeddedDocumentField`) used appropriately?
- **Query Performance**: Are database queries optimized (e.g. using `only()`, `exclude()`, or `.aggregate()`) to avoid loading unnecessary data or causing N+1 query problems?
- **State Management**: Is `flask.g` or `flask.session` used correctly for request-scoped state without leaking data between requests?

## 2. Security & Access Control

- **Authentication / Authorization**: Are routes properly protected using `@login_required` or custom decorators? Does the PR correctly check if a user is an admin or participant before allowing access to CTF flags/hints?
- **Flag Security**: Are CTF flags securely compared (e.g. using constant-time string comparison or hashed checks) to prevent timing attacks?
- **Input Validation**: Are forms and inputs validated using `flask-wtf` or equivalent before processing? Is there any risk of NoSQL injection or XSS (Cross-Site Scripting)?
- **Data Privacy**: Are sensitive fields (passwords, admin-only notes) excluded when serializing user or challenge objects for public endpoints?

## 3. Frontend & Framework Standards

- **Templates (Jinja2)**: Are Jinja2 templates using inheritance (`{% extends %}`) and macros properly to avoid code duplication?
- **Static Assets**: Are static files (JS, CSS, images) properly referenced using `url_for('static', filename='...')`?
- **Accessibility (a11y)**: Does the UI support keyboard navigation? Are semantic HTML and proper ARIA attributes used?
- **Form CSRF**: Do all forms include a CSRF token (`{{ form.csrf_token }}`) to prevent Cross-Site Request Forgery?

## 4. Code Quality & General Principles

- **Python / PEP 8**: Does the code follow standard Python conventions? Are type hints used where appropriate?
- **DRY & YAGNI**: Is the code duplicating existing utility functions? Is it over-engineering features (You Aren't Gonna Need It)?
- **Error Handling**: Are errors caught and handled gracefully (e.g. custom error pages for 404, 500)? Is the user informed of failures appropriately?
...
- **Leftover Code**: Has the code been cleaned of leftover `print()`, `pdb.set_trace()`, and commented-out code before opening the PR?
...
3. **Load related skills** (read and apply before concluding):
   ...
   - `flask-bestpractices` — Flask, Jinja2, Blueprints
   - `mongodb-bestpractices` — MongoEngine, data modeling, query optimization
   - `security-rbac-bestpractices` — CTF challenge security, roles, input validation
   - `testing-bestpractices` — Pytest, DoD
